...
// --------------------------------------------------------
   ...
   if(!isAdj2) {
    if(((lpWorkBuffer->fieldType)==CELLBOX)||((lpWorkBuffer->fieldType)==CELLBOXDIVIDED)||((lpWorkBuffer->fieldType)==MULTIBOX)) {
         add=0;
         for(i=1;i<lpWorkBuffer->lengthTraceL;i++) {
             if(lpWorkBuffer->arrCellP[(lpWorkBuffer ->lenLeftC)-1].rT.x<(lpWorkBuffer->traceL[i])) add++;
             }
         if(add>(lpWorkBuffer->lengthTraceL)/3) return(-200);

         if(!apostrof(lpWorkBuffer,1,kindD)) {
                return(-200);
                }
         }
...
/***************************************************************************
 *                                                                         *
 *  PURPOSE      :first Max and Min indexis for current segment            *
 ***************************************************************************/
