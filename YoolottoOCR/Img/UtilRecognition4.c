...
void sepBoxes( STRUCTWB *lpWorkBuffer)
{
   int x1,x2,y1,y2;
   int len, tH, end, end2;
   int i, k, cc, max1, max2;
   int cutM, marg, margCut, Ok, margLR;
   int   avArea;

   cutM = lpWorkBuffer->nOfSpConstSegm;
   marg    = 16;
   margCut = 2 + cutM;
   margLR  = 2;

    y1 = lpWorkBuffer->traceL[0];
    y2 = y1 + lpWorkBuffer->lengthTraceL;
    x1 = lpWorkBuffer->traceL[1];
    x2 = lpWorkBuffer->traceC[1];
    hist_h(lpWorkBuffer,x1,y1,x2,y2);       // horizontal histogram
    tH  = (x2 -x1)/2;
    smHist2(tH);  // smooth histogram
...
// top
    ...
    for(i=1;i<end;i++) {
       if(hntHist[i] > tH) {
            cc++;
            if(hntHist[i] > max1) max1 = hntHist[i];
            }
       else
          if(cc > 0)
                break;
    ...
    if(cc < 3) {
          ...
          for(k=i+1;k<end2;k++) {
             if(hntHist[k] > tH) {
                  cc++;
                  if(hntHist[k] > max2) max2 = hntHist[k];
                  }
             else
                if(cc > 0)
                    break;
          ...
          if((cc > 1) && (max2 > max1)) {i = k;max1 = max2;}
          }

    if(i > 8)
       if(max1 < (x2 -x1)*2/3)  i = end;

    i += cutM;
    if(i < end)   {
         lpWorkBuffer->traceL[0] += i;
         lpWorkBuffer->lengthTraceL -= i;
         lpWorkBuffer->traceC[0] += i;
         lpWorkBuffer->lengthTraceC -= i;

         }
    else {
         lpWorkBuffer->traceL[0] += margCut;
         lpWorkBuffer->lengthTraceL -= margCut;
         lpWorkBuffer->traceC[0] += margCut;
         lpWorkBuffer->lengthTraceC -= margCut;
         }
...
// Bottom
    ...
    histVer(lpWorkBuffer,(int *)hntHist,x1,x2,y1,y2); // vertical histogram
    tH  = (y2 -y1)/2;
    smHist2(tH);  // smooth histogram
    len = hntHist[0];
    end = len/2;
// left side
    ...
    i += cutM;
    if(i < end)   {
         for(k=1;k<=lpWorkBuffer->lengthTraceL;k++)
             lpWorkBuffer->traceL[k] += i;
         }
    else {
         for(k=1;k<=lpWorkBuffer->lengthTraceL;k++)
             lpWorkBuffer->traceL[k] += margCut;
         }
...
// right side
    ...
    i = len - i + 1;
    if(i < end)   {
         for(k=1;k<=lpWorkBuffer->lengthTraceC;k++)
             lpWorkBuffer->traceC[k] -= i;
         }
    else {
         for(k=1;k<=lpWorkBuffer->lengthTraceC;k++)
             lpWorkBuffer->traceC[k] -= margCut;
         }
   avArea  =  (lpWorkBuffer->traceC[1]-lpWorkBuffer->traceL[1])*lpWorkBuffer->lengthTraceL;
   avArea  /= 2;
...
int  massBox(STRUCTWB *lpWorkBuffer)
{
  int tHm;
  int i, x, y, rm;

   tHm=(int)((float)20*((float)(lpWorkBuffer -> Dpi.x)/(float)300.0));
 ...
 *-----------------------------------------------------------------*/
 ...
 int smSignBox(STRUCTWB *lpWorkBuffer)
 {
    int x1,x2,y1,y2;
    int len, tH, i;
    int yT, yB, hh, ww;
    ...
    float ratio;
     ...
     yT = len;
     for(i=1;i<len;i++)
        if((hntHist[i] > tH) && (hntHist[i+1] > tH)) {
           yT = i;
           break;
           }
    ...
    ratioA = (int)(((float)(ww*hh)/(float)lpWorkBuffer->avArea)*(float)100);
    if( ww > 0)
        ratio = (float)hh/(float)ww;
    else
       return(5);
    if(ratioA < 5)
            return(5);   // trash
  ...
  // segmentation for proportional font
                  *lpLwidth= (lpWorkBuffer->  lettWidth);
                      ret=segFont((IMAGEL *)&imageForRec,
                               ...
                               (int  *)&indxMarkC,(long *)&Cost[0],(int *)pOutT,0,fieldName);
                      if(ret<0) return(ret);
  ...
  // segmentation for proportional font
                  *lpLwidth= (lpWorkBuffer->  lettWidth);
                      ret=segFont((IMAGEL *)&imageForRec,
                               ...
                               (int  *)&indxMarkC2,(long *)&Cost2[0],(int *)pOutT,0,fieldName);
                      if(ret<0) return(ret);
...
// if(charSaveIndex == 2){
//	 int  lenHistTmp;
//   lenHistTmp=(lpWorkBuffer->curLine[i]).x2-(lpWorkBuffer->curLine[i]).x1+1;
//   saveCSVFile(hntHist, lowProf, upProf, lenHistTmp, "hntHist", "lowProf", "upProf","profile");
//   }
                k=SegmProfiles(lpWorkBuffer,i,k);
/*
      if(charSaveIndex == 2){
   		 int kki, rm[100], gm[100], dm[100];
   		 for(kki = 0;kki<lpWorkBuffer->markNumber;kki++){
   			rm[kki] = (lpWorkBuffer->mark[kki]).r_m-(lpWorkBuffer->curLine[i]).x1;
   			gm[kki] = (lpWorkBuffer->mark[kki]).g_m;
   			dm[kki] = (lpWorkBuffer->mark[kki]).d_m;
   		 }
  	   saveCSVFile(rm, gm, dm, lpWorkBuffer->markNumber, "rm", "gm", "dm", "mark");
       ...
       lpWorkBuffer->shfH = shfX;
             ...
             if(((lpWorkBuffer->flgUNP)==4) || ((lpWorkBuffer->flgUNP)==5)) {
                 if((lpWorkBuffer->flgUNP)==4) {
                     ...
                     *lpLwidth=0;
 //                *lpLwidth=(lpWorkBuffer->  lettWidth);
//printf("  1 segFixedPitch; *lpLwidth= %d \n", *lpLwidth);
                   ret=segFixedPitch((WORD)lpWorkBuffer -> Dpi.x,(WORD)lpWorkBuffer -> Dpi.y,
                                 (int *)&hntHist[begHOtp],endHOtp-begHOtp+1,(int *)lpLwidth,
                 ...
                 *lpLwidth=(lpWorkBuffer->  lettWidth);
//printf("  2 segFixedPitch; *lpLwidth= %d \n", *lpLwidth);
                   ret=segFixedPitch((WORD)lpWorkBuffer -> Dpi.x,(WORD)lpWorkBuffer -> Dpi.y,
                                 (int *)&hntHist[begHOtp],endHOtp-begHOtp+1,(int *)lpLwidth,
...
// printf("  3 segFont; *lpLwidth= %d \n", *lpLwidth);
                 ...
                 if(ret<0) return(ret);
                 (lpWorkBuffer->  lettWidth)=*lpLwidth;
//printf("  after segFixedPitch; *lpLwidth= %d \n", *lpLwidth);
...
//printf("  after segFixedPitch; *lpLwidth= %d \n", *lpLwidth);
                     ...
                     *lpLwidth=(lpWorkBuffer->  lettWidth);
// printf("  5 segFixedPitch; *lpLwidth= %d \n", *lpLwidth);
                      ret=segFixedPitch((WORD)lpWorkBuffer -> Dpi.x,(WORD)lpWorkBuffer -> Dpi.y,
                                   (int *)&hntHist[begHOtp],endHOtp-begHOtp+1,(int *)lpLwidth,
                                   (int *)markConst,(int *)&indxMarkC,(long *)&Cost[0],0);
                      if(ret<0) return(ret);
 ...
 /*  one step segmentation for unproportional fonts */
      ...
      if( ((lpWorkBuffer->  lettWidth)>0)&&((lpWorkBuffer->  Method)==1) ) {
        if(kindD >3) {
         ...
         if(lpWorkBuffer -> segmType  == 55) {
              int n;
              if(!lpWorkBuffer ->nOffCut600) {
                n  = (7*lpWorkBuffer->lettWidth + 5)/10;
                if(markConst[indxMarkC - 2] - markConst[indxMarkC - 1] < n)
                  indxMarkC--;
                if(markConst[0] - markConst[1] < n)
                  shiftSegm((int*)markConst, 0, (int*)&indxMarkC);
                  }
              else {
                if(lpWorkBuffer ->nOffCut600 < indxMarkC-1) {
                   if(markConst[indxMarkC - 2] - markConst[indxMarkC - 1] <
                      markConst[0] - markConst[1]) {
                      indxMarkC--;
                      if(lpWorkBuffer ->nOffCut600 < indxMarkC-1)
                          shiftSegm((int*)markConst, 0, (int*)&indxMarkC);
                      }
...
//printf("  indxMarkC= %d, begHOtp= %d, (lpWorkBuffer->curLine[0]).x1+begHOtp-1=%d \n", indxMarkC, begHOtp, (lpWorkBuffer->curLine[0]).x1+begHOtp-1);
//         if((lpWorkBuffer->flgUNP)==2) {
       	 if(strcmp(fieldName, "NUMBERS") == 0) {
        	 cleanCuts((IMAGEL *)&imageForRec, (lpWorkBuffer->recChar), (int  *)&hntHist[begHOtp], endHOtp-begHOtp+1, sizeW);
         }
         ...
         if((lpWorkBuffer -> segmType == 57) && (ret <=1000) &&
         ...
         if((lpWorkBuffer -> segmType == 57) && (ret > 1000)) // separet cellboxes
                                    ret=sendNextFont(lpWorkBuffer, "NO");
         if((lpWorkBuffer -> segmType == 57) && (lpWorkBuffer->lengthTraceL <(lpWorkBuffer->  lettWidth)/2))
                                    ret=sendNextFont(lpWorkBuffer, "NO");
         if(lpWorkBuffer -> segmType == 57) {
                     sepBoxes(lpWorkBuffer);
                     if(massBox(lpWorkBuffer)) return(1001);
                     if((kindD == 2) || (kindD == 0)) {
                         return(4);
                       }
                     }
 ...
 /***************************************************************************
  *  PURPOSE      :searching for lokal min/max of profiles                  *
  *                                                                         *
  ***************************************************************************/
...
void saveCSVFile(int* v1, int* v2, int *v3, int len, char* t1, char* t2, char* t3, char *name){
   ...
   strcpy(fileName, "c:\\Users\\Wicek\\YoolottoVin\\work\\");
    ...
    for(i=0;i<indxM-1;i++) {
          ...
          if(mass > 6) {
             ...
             err = extractChar(pImage, angle, shift,
                            ...
                            pBuffer, pWidth, pHeight);
             if(err < 0){ conf = 0;
//         printf("-----------------------------  err = %d, markC[i1]= %d, markC[i2]= %d, pWidth= %d, pHeight= %d \n", err, markC[i1], markC[i2], *pWidth, *pHeight);
             }
             ...
             cc++;
             if(conf > TH) ccGood++;
//printf("-----------------------------  i1= %d, i2= %d, markConst[i1]= %d, markConst[i2]= %d, cnf= %d \n", i1, i2, markConst[i1], markConst[i2], conf);
          }
