...
(WORD wDpiX,                /*X-resolution                 */
 ...
 if(cw > n-3)
     ...
     *(pCut+1L) = 0;
     *pNCut = 2;
     *pCost = var2[0] + var2[n1];
     return(0);
    }

 for(i=0; i<=cw+2; i++) {ccc[i] = var2[i]; ind[i] = -1;}

 for(i=cw+3; i<=n1; i++)
    {j = i-cw-3;

     cji = ccc[j] - abs(var2[j] - var2[i])/2;
     maxL = cji; ind[i] = j++;
     cji = ccc[j] - abs(var2[j] - var2[i])/2;
     if(cji>=maxL) {maxL = cji; ind[i] = j;} j++;
     cji = ccc[j] - abs(var2[j] - var2[i])/2;
     if(cji>=maxL) {maxL = cji; ind[i] = j;} j++;
     cji = ccc[j] - abs(var2[j] - var2[i])/2;
     if(cji>=maxL) {maxL = cji; ind[i] = j;} j++;
 ...
 j = n-cw-3;
 maxL = ccc[j]; ind[n] = j;
 for(j=n-cw-2; j<=n1; j++) {if(ccc[j]>maxL) {maxL = ccc[j]; ind[n] = j;}}
 *pCost = maxL;

 i=0;
 xxx[i] = ind[n];
 while((j = xxx[i]) >= 0)
    {xxx[i+1] = ind[j]; *(pCut+(long)i) = j;
     i++;
     }
 *pNCut = i;
...
int * pH,              /*pointer to the line histogram */
int n,                     /* size of histogram            */
int * pCw,             /* pointer to the cell width    */
int * pCut,            /* pointer to the sequence of
                              x-coordinates of boundary
                              left points                 */
int * pWidth,          /* pointer to the sequence of
                              boundaries' width            */
int * pHeight,         /* pointer to the sequence of
                              boundaries' height           */
int * pNCut,            /* pointer to number of cuts    */
long * pCost,          /* pointer to cost of partition */
int version                /* version number (temporary)   */
...
{int err;
 ...
 var1[0] = *pH;
 for(i=1; i<=n1; i++) var1[i] = *(pH+(long)i) - *(pH+(long)(i-1));
 var1[n] = -(*(pH+(long)n1));
 ...
 d = (d*wDpiX*2 + 200)/(2*200);
 for(i=0; i<=d-1; i++)
    {max = -1;
     for(j=1; j<=d; j++)
        {if(i-j>=0)
            {m = var1[i-j] - var1[i+j];
             if(m>max) {max = m; var2[i] = m; dMaxSeg[i] = 2*j;}
            }
         if(i-j+1>=0)
            {m = var1[i-j+1] - var1[i+j];
             if(m>max) {max = m; var2[i] = m; dMaxSeg[i] = 2*j-1;}
            }
        }
    }
 for(i=d; i<=n-d; i++)
 ...
 if(*pCw == 0)
   {
    ...
    for(cw=cwMin; cw<=cwMax; cw++)
       {err =
           segBoxesPartition
               (wDpiX,wDpiY,pH,pVar2,n,cw,pCut,pNCut,pCost,version);
        if(err<0) return(err);
 ...
 err =
      segBoxesPartition
         (wDpiX,wDpiY,pH,pVar2,n,cwM,pCut,pNCut,pCost,version);
         if(err<0) return(err);
 ...
 for(i=0; i<nC; i++)
    {pos = *(pCut+(long)i);
     *(pCut+(long)i) = pos - dMaxSeg[pos]/2;
     *(pWidth+(long)i)  = dMaxSeg[pos];
     *(pHeight+(long)i) = 0;
    }
...
void lineRec(int *hist, int lenH, STRUCTWB *lpWorkBuffer, int indFr){
 int i, j, indT, indB, y1, dd, srI;
 int indT2, indB1, dd1;
 long sr;
     y1=(lpWorkBuffer->begRect.y)+(lpWorkBuffer->OffH);
     for(i=0;i<(lpWorkBuffer->infFrame[indFr]).nLines;i++) {
          indT  = (lpWorkBuffer->line[indFr]).y1[i] - y1;
          indB  = (lpWorkBuffer->line[indFr]).y2[i] - y1;
          indT  = max(1,indT);
          indB  = min(lenH-1,indB);
          indT2 = indT + 2;
          indB1 = indB - 1;
          dd   = indB1 - indT2;
          if(dd > 25) {
                 sr = 0L;
                 for(j=indT2;j<indB1;j++) {
                         sr += (long)hist[j];
                         }
                 srI = (int)(sr/dd);
                 dd = indT;
                 ...
                 if((dd != indT) && (indB - dd >= 18))  {
                         if(i == 0)  {
                                if(indB - dd > 22) {
                                      ...
                                      for(j=dd+1;j<=dd+3;j++) {
                                       ...
                                       if(dd1 > dd) {
                                           if((hist[dd1+1] > srI) || (hist[dd1+2] > srI))
                                                               dd = dd1;
                                           }
                                       }
                                }
                         if(i == 0)
                                 dd++;
                         (lpWorkBuffer->line[indFr]).y1[i] = dd + y1;
                         }
                 }
         }
 ...
 *                                                                         *
 ***************************************************************************/
int linesTxtSp(int *hist, int lenH, STRUCTWB *lpWorkBuffer, int lineH, int indFr){

  int   res, marg, tH, tHd, d, nL, i, flgUp, top = 0, avM, j;
  int   y1, jj, kk;

  if((indFr < 2) || (indFr>3)) {return(-1);}
        ...
        avM = 0;
        for(i=marg;i<lenH-marg;i++) {
                if(avM < hist[i]) {avM = hist[i];}
                }
        tH=avM-res;
        ...
        if(nL == 0) {return(-1);}
        for(i=0;i<nL;i++) {
                d = (tabLine[1][i]-tabLine[0][i]) - lineH;
                if(d < 0) {d = -d;}
                if(avM > d) {avM = d;}
                tabLine[2][i] += 35;
                }
        ...
        for(i=0;i<nL;i++) {
                ...
                d -= avM;
                if(d < 0) {d = -d;}
                if( d > 2) {tabLine[2][i] = 0;}
                }
        jj = -10;kk = 0;
        while(1) {
          avM = 10000;j = -1;
          for(i=0;i<nL;i++) {
                if((tabLine[2][i] > 0) && (avM >= tabLine[0][i]) && (tabLine[0][i] - jj > 6)) {
                     avM = tabLine[0][i];
                     j = i;
                     }
                }
          ...
          if(j < 0) {break;}
          (lpWorkBuffer->line[indFr]).y1[kk]      = max(tabLine[0][j]+y1-2,y1);
          (lpWorkBuffer->line[indFr]).y2[kk]      = min(tabLine[1][j]+y1+3,y1+(lpWorkBuffer->Height)-1);
          kk++;
          (lpWorkBuffer->infFrame[indFr]).nLines  = kk;
          tabLine[2][j] = 0;
          if(kk > 3) {break;}
          jj = avM;
          }
