...
/*
 char* lpDUResultBuffer = &DUResultBuffer[0];
 char* lpDResultBuffer  = &DResultBuffer[0];
 char* lpUResultBuffer  = &UResultBuffer[0];
 char* lpLResultBuffer  = &LResultBuffer[0];
 char* lpUCand1Buffer  = &UCand1Buffer[0];
 char* lpLCand1Buffer  = &LCand1Buffer[0];
 char* lpUCand2Buffer  = &UCand2Buffer[0];
 char* lpLCand2Buffer  = &LCand2Buffer[0];
*/
...
// printf("  originX0= %d, originY0= %d, widthRec0=%d, heightRec0= %d, shift= %ld \n", originX0, originY0, widthRec0, heightRec0, shift);
// sprintf(buff,"  originX0= %d, originY0= %d, widthRec0=%d, heightRec0= %d, shift= %ld \n", originX0, originY0, widthRec0, heightRec0, shift);
// saveLog("", buff);
...
// printf("  wDpiX= %d, wDpiX0= %d, wCellWidth= %d \n", wDpiX, wDpiX0, wCellWidth);
// printf("  wDpiY= %d, wDpiY0= %d, wLineHeight= %d \n", wDpiY, wDpiY0, wLineHeight);
...
// printf("  wFieldType= %d, wFieldAlphabet= %d, wCellConst=%d, wCellWidth= %d, wHorLineRem= %d \n", wFieldType, wFieldAlphabet, wCellConst, wCellWidth, wHorLineRem);
// printf("  wVerLineRem= %d, wTextLineRec= %d, wLineHeight=%d, wHorSpecksSize= %d, wVerSpecksSize= %d \n", wVerLineRem, wTextLineRec, wLineHeight, wHorSpecksSize, wVerSpecksSize);
// printf("  wIgnSubSigns= %d, wIgnSubSignsPar= %d, wDictIndex=%d, wContext= %d, wConfThreshold= %d \n", wIgnSubSigns, wIgnSubSignsPar, wDictIndex, wContext, wConfThreshold);
...
// else if ((wFieldAlphabet1 >= 42)&&(wFieldAlphabet1 <= 45)) nNormVersion = 1;
 else nNormVersion = 1;
...
// printf("  lpCand1Buffer= %ld, lpCand2Buffer= %ld,\n", (long)lpCand1Buffer, (long)lpCand2Buffer);
...
//  printf("  lpImageL -> iWidth= %d, lpImageL -> iHeight= %d, lpImageL -> iDpiX=%d, lpImageL -> iDpiY= %d \n", lpImageL -> iWidth, lpImageL -> iHeight, lpImageL -> iDpiX, lpImageL -> iDpiY);
...
//   sprintf(buff,"  recField; err=  %d \n", err);
//   saveLog("", buff);

//     printf("  recField; err=  %d \n", err);
//     return 0;
      if(err < 0) return(err);
     }
...
//  bContextOn = (lpContext -> iContextOn);
...
// printf("  xOrigin0= %d, yOrigin0= %d, xCorner0= %d, xCorner0= %d \n", (int)xOrigin0, (int)yOrigin0, (int)xCorner0, (int)yCorner0);
//  sprintf(buff,"  xOrigin0= %d, yOrigin0= %d, xCorner0= %d, xCorner0= %d \n", (int)xOrigin0, (int)yOrigin0, (int)xCorner0, (int)yCorner0);
//  saveLog("", buff);
...
/*
  if(wAlphaIndx1 > 10000)
       lpNCRStruct = GetNCRStructX(wAlphaIndx1);
       if(lpNCRStruct != NULL) {
            if(lpNCRStruct->Version == 0) wCharTypeForSegm = 0;
            else                          wCharTypeForSegm = 6;
       }
*/
//printf("  wCharTypeForSegm= %d, bRecContinueFd= %d \n", wCharTypeForSegm, bRecContinueFd);
//  return 0;

//  sprintf(buff,"  wCharTypeForSegm= %d, bRecContinueFd= %d \n", wCharTypeForSegm, bRecContinueFd);
//  saveLog("", buff);
...
// printf("  position= %ld \n", position);
// printf("  xOrigin0= %d, yOrigin0= %d, xCorner0= %d, yCorner0= %d \n", (int)xOrigin0, (int)yOrigin0, (int)xCorner0, (int)yCorner0);
// printf("  xOriginFd= %d, yOriginFd= %d, xCornerFd= %d, yCornerFd= %d \n", (int)xOriginFd, (int)yOriginFd, (int)xCornerFd, (int)yCornerFd);
/*
  sprintf(buff,"  position= %ld \n", position);
  saveLog("", buff);
  sprintf(buff,"  xOrigin0= %d, yOrigin0= %d, xCorner0= %d, yCorner0= %d \n", (int)xOrigin0, (int)yOrigin0, (int)xCorner0, (int)yCorner0);
  saveLog("", buff);
  sprintf(buff,"  xOriginFd= %d, yOriginFd= %d, xCornerFd= %d, yCornerFd= %d \n", (int)xOriginFd, (int)yOriginFd, (int)xCornerFd, (int)yCornerFd);
  saveLog("", buff);
*/

// test-------------------------------------------------
//  testFieldBufferBMP =  ticketDataPtr->fieldBMPPtr;
...
//  saveLog("", buff);
  ...
  while(end == 0)
       {
//	  sprintf(buff,"  recNext \n");
//	  saveLog("", buff);

	  retValue = recNext(ticketDataPtr);

//	  sprintf(buff,"  retValue= %d \n", retValue);
//	  saveLog("", buff);
...
/*
         ...
         alphabet = (LPSTR)&alphabetBuffer[0];
         pHeader = (LPBYTE)(lpNCRStruct->char_list);
         nChar = (int)lpNCRStruct->Total_Classes;
         counter = 0;
         if(nChar > 0) {alphabet[0] = pHeader[0]; counter = 1;};
         for(i=1;i<nChar;i++)
              if(pHeader[i] != pHeader[i-1])
                   {alphabet[counter] = pHeader[i]; counter++;};
         alphabet[counter] = 0;
         nChar = counter;
    }
*/

/*    04-27-97 New normalization.
   Parameters should be retrieved from NNB and NCR files.
*/
...
/*
           ...
           size         = lpNCRStruct->Width;
           nNormVersion = lpNCRStruct->Version;
           delta        = lpNCRStruct->Delta;
           factor       = lpNCRStruct->Factor;
...
//  sprintf(buff," before initFieldR  \n");
//  saveLog("", buff);
 charCounter  = 0;
 err =  initFieldR
           (
...
//printf("  retSegm= %d, charSaveIndex= %d, lineSaveIndex= %d; \n", retSegm, charSaveIndex, lineSaveIndex);
   ...
   if((retSegm != 0)&&(retSegm != 3)&&(retSegm <= 1000))
   {
    ...
    if((wCharTypeForSegm>= 4)) {
       if((40 <= retSegm)&&(retSegm < 100)) {
         ...
         if(strcmp(ticketDataPtr->fieldAddress->name, "NUMBERS") == 0 && charSaveIndex > 2 && charSaveIndex < 20){
...
/*
 ...
 strcat(imgName, "_");
 strcat(imgName, ticketDataPtr->fieldAddress->name);
     if(strcmp(ticketDataPtr->fieldAddress->name, "NUMBERS") == 0 && lineSaveIndex == 1)
...
/*
     if(strcmp(ticketDataPtr->fieldAddress->name, "NUMBERS") == 0){
printf("  charSaveIndex= %d, lineSaveIndex= %d \n", charSaveIndex, lineSaveIndex);
printf("  err= %d, flag= %d \n", err, flag);
printf("  iIgnSubSignsPar= %d, pResult[3]= %c \n", iIgnSubSignsPar, pResult[3]);
printf("  wAlphaIndx1= %d, pResult[5]= %c \n", wAlphaIndx1, pResult[5]);
    }
*/
     if((err < 0)||(flag == 0)) {
               *lpCand1Buffer++ = 32;
...
/*
    if(bContextOn&&(len >= 2L)) {
      err = contextq(lpLastWord1,
                      lpLastWord2,
                      lpLastWord,
                      (short int)len,
                      wDictIndx,
        ...
        if(err < 0)
           {for(j1=0; j1<(int)len; j1++)
                  *(lpLastWord+(long)j1) =  *(lpLastWord1+(long)j1);
           }
      }
*/
...
void saveCharacter(LPBYTE buffer, WORD width, WORD height, int lineIdx, int charIdx, char* imgName){
        ...
        charBMP = calloc(size, sizeof(BYTE));
        ...
        *(LPDWORD)(qd + 1) = 0x00ffffff;
        ...
        {char name[200];
         ...
         strcpy(name, imgName);
          ...
          strcat(name, strNum);
          strcat(name, ".bmp");
          saveBMPbuffer (charBMP, name, "c:\\Users\\Wicek\\YoolottoVin\\output\\");
        }
...
// unsigned short t;
 ...
 int i;
 int conf;
// unsigned char annResult[8];
// LPBYTE nnResult = &annResult[8];
...
//              if(err < 0) return(-301);
 ...
 if(index<10000)
    {
/*
	 if((index < 32)||(index > 54)) index = index;
     else {
    	 index = index - 32;
           t = 100;
           err = -77;
...
/*
    ...
    {
     long Num_Cand = 5;
     BYTE NCRSubsetTag[128];
     LPSTR NCRSubset = (LPSTR)&NCRSubsetTag[0];
     BYTE masc[] = {1, 2, 4, 8, 16, 32, 64, 128};
     int len, i, counter;
     BYTE NCRChar_listTag[512];
     LPSTR NCRChar_list = (LPSTR)&NCRChar_listTag[0];
     int NCRTotal_Classes = (int)(lpNCRStruct->Total_Classes);

     for(i=0;i<NCRTotal_Classes;i++) NCRChar_list[i] = lpNCRStruct->char_list[i];
     NCRChar_list[NCRTotal_Classes] = 0;

     lpNCRResults->indexes = (long far *)&NCRIndexes[0];
     lpNCRResults->confidences = (double far *)&NCRConf[0];

     len = lstrlen(alphabet);
     counter = 0;
     if(pSubsetMap != NULL) {
          for(i=0; i<len; i++)
              if(pSubsetMap[i/8]&masc[i%8])
                   NCRSubset[counter++] = alphabet[i];
     }
     else  for(i=0; i<len; i++) NCRSubset[counter++] = alphabet[i];
     NCRSubset[counter] = 0;

     err =  recCharNCR(lpNCRStruct, normPict, NCRSubset, lpNCRResults, Num_Cand);
            if(err < 0) return(-306);

     nChar = lstrlen(NCRSubset);
     ...
     for(i=0; i<nCand; i++) {
          pResult[2+2*i+1] = NCRChar_list[lpNCRResults->indexes[i]];
          pResult[2+2*i] = (int)floor((lpNCRResults->confidences[i])*100 + 1);
     }
     if(nCand==1)
        {
         pResult[2] = 100;
...
/********************************************************************/
/*																	*/
/*	pictWidthHeightSizeDeltaFactor()								*/
/*																	*/
/*	RETURN: returns 0.												*/
/*																	*/
/*	NOTE: sets width and height of character and size of normalized	*/
/*		character. size should be equal to 15, delta should be		*/
/*		equal to 1. factor should be equal to 10 					*/
/*																	*/
/********************************************************************/
...
/*	RETURN: returns 0												*/
/*																	*/
/*	NOTE: calculate L1 distance between normalized characters 		*/
/*																	*/
/********************************************************************/
...
/*	RETURN: returns 0.												*/
/*																	*/
/*	NOTE: sets bitmap version for character.		 				*/
/*		mapVersion should be equal to 0 for Training Tool			*/
/*		and 1 for TransForm											*/
/*																	*/
/********************************************************************/
...
/*	RETURN: returns 0												*/
/*																	*/
/*	NOTE: sets normalization version.				 				*/
/*		should be equal to 0 for handprinted characters (NTW).		*/
/*		should be equal to 1 for machine printed characters			*/
/*		(both NTW or NNB)											*/
/*																	*/
/********************************************************************/
...
/*	pictFromNormal()												*/
/*																	*/
/*	RETURN: returns 0, if OK. Otherwise returns PAR_ERR, CHAR_ERR,	*/
/*		CHARSIZE_ERR or MASS_ERR									*/
/*																	*/
/*	NOTE: calculates normalized character			 				*/
/*																	*/
/********************************************************************/
...
/*	getMoments()													*/
/*																	*/
/*	RETURN: returns 0, if OK. Otherwise returns CHARSIZE_ERR,		*/
/*			or MASS_ERR  											*/
/*																	*/
/*	NOTE: calculates central moments of 0, 1, an 2 orders 			*/
/*																	*/
/********************************************************************/
...
int normalizeM(LPPATT lpPatt, LPPATT lpNormPatt, int delta, int factor, int mapVersion, int normVersion){
 ...
 if(!pMemNorm)
	 pMemNorm = calloc(0xFFFF, sizeof(BYTE));
...
int getMomentsFD(LPPATT lpPatt,  LPMOMENTSFD lpMoments, int mapVersion){
 ...
 if(!pMemMomentsFD)
	 pMemMomentsFD = calloc(12*CHARSIZE_G, sizeof(BYTE));
