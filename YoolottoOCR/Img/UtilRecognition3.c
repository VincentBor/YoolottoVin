...
/***************************************************************************
 *									   *
 *  FUNCTION     :int sizeSpot()                                           *
 *                                                                         *
 *  PURPOSE      :find size black spot                                     *
 *                                                                         *
 *  RETURN VALUE : 0 no                                                    *
 *                 1 yes                                                   *
 *                                                                         *
...
/***************************************************************************
 *  PURPOSE      :calculate lines for indFr frame  of the rectangle        *
 *                MULTILINES                                               *
***************************************************************************/
 ...
 *                                                                         *
 ***************************************************************************/
void linesFrame(int indFr, STRUCTWB *lpWorkBuffer, int ttH){
   int i, len, flg_g, f_time, count_0, y1, indL, tH, delt, tH1, tHC;
       len=hntHist[0];
       tH=TH_LEVEL_HIST;
       flg_g=0;
       count_0=0;
       ...
       switch(ttH) {
        case 0:
           tH1=max((lpWorkBuffer->  lettWidth)/3,10);
           tHC=TH_C_LEVEL_HIST-1;
           break;
        case 1:
           tH1=2;
           tHC=0;
           ...
           break;
        case 7:
           tH1=max((lpWorkBuffer->  lettWidth)/10,4);
           tHC=TH_C_LEVEL_HIST-1;
           break;
        default:
           tH1=max((lpWorkBuffer->  lettWidth)/3,10);
       ...
       while(i<len) {
             ...
             if(hntHist[i]>tH)  {
                    ...
                    if( (flg_g>tHC)&&(count_0>tH1-1) ){
                           if(ttH==1) delt=0;
                           else delt=endLine(i,len);
                           i+=(delt-count_0+2);
                           (lpWorkBuffer->line[indFr]).y2[indL]=min(i+y1,(lpWorkBuffer->begRect.y)+(lpWorkBuffer -> Height)-1);
                           indL++;
                           if(indL>=MAXLINES) {
...
int linesOrder(STRUCTWB *lpWorkBuffer){
 int frN, i, j, minT, d, tH;
     tH = 7;
     frN=(lpWorkBuffer->frNumber);
     (lpWorkBuffer->infFrame[0]).x2 = (lpWorkBuffer->infFrame[frN-1]).x2-16;
     (lpWorkBuffer->frNumber) = 1;

     if(frN+1<MAXFRAMES) {
        for(i=0;i<(lpWorkBuffer->infFrame[0]).nLines;i++) {
           (lpWorkBuffer->line[frN]).y1[i] = (lpWorkBuffer->line[0]).y1[i];
           (lpWorkBuffer->line[frN]).y2[i] = (lpWorkBuffer->line[0]).y2[i];
           }
        (lpWorkBuffer->infFrame[frN]).nLines = (lpWorkBuffer->infFrame[0]).nLines;
        frN++;
        }

     (lpWorkBuffer->infFrame[0]).nLines = 0;

     while(1) {
        minT = 10000;j = -1;
        for(i=1;i<frN;i++) {
          if((lpWorkBuffer->infFrame[i]).nLines>0) {
            if(minT > (lpWorkBuffer->line[i]).y1[0]) {
                minT = (lpWorkBuffer->line[i]).y1[0];
                j = i;
                }
            }
          }
        ...
        i = (lpWorkBuffer->infFrame[0]).nLines;
        d = 10;
        if(i > 0) {
           d = (lpWorkBuffer->line[j]).y1[0] -(lpWorkBuffer->line[0]).y1[i-1];
           }

         if(d > tH) {
            (lpWorkBuffer->line[0]).y1[i] = (lpWorkBuffer->line[j]).y1[0];
            (lpWorkBuffer->line[0]).y2[i] = (lpWorkBuffer->line[j]).y2[0];
            (lpWorkBuffer->infFrame[0]).nLines++;
            }

        for(i=1;i<(lpWorkBuffer->infFrame[j]).nLines;i++) {
           (lpWorkBuffer->line[j]).y1[i-1] = (lpWorkBuffer->line[j]).y1[i];
           (lpWorkBuffer->line[j]).y2[i-1] = (lpWorkBuffer->line[j]).y2[i];
           }
        (lpWorkBuffer->infFrame[j]).nLines--;
        }
     return(-1);
}
void changeLineOrder(STRUCTWB *lpWorkBuffer, int lNum){
  int  i, j, k, tmp, frN, nLines;
     frN=(lpWorkBuffer->frNumber);
     for(i=0;i<frN;i++) {
         nLines = (lpWorkBuffer->infFrame[i]).nLines;
         if(nLines > 1) {
           for(j=0;j<nLines/2;j++) {
              k   = nLines - j - 1;
              tmp                         = lpWorkBuffer->line[i].y1[j];
              lpWorkBuffer->line[i].y1[j] = lpWorkBuffer->line[i].y1[k];
              lpWorkBuffer->line[i].y1[k] = tmp;
              tmp                         = lpWorkBuffer->line[i].y2[j];
              lpWorkBuffer->line[i].y2[j] = lpWorkBuffer->line[i].y2[k];
              lpWorkBuffer->line[i].y2[k] = tmp;
              }
           }

        if((lNum >= 0) && (nLines > lNum)) {
          (lpWorkBuffer->infFrame[i]).nLines = lNum;
          for(j=lNum;j<nLines;j++) {
              lpWorkBuffer->line[i].y1[j] = 0;
              lpWorkBuffer->line[i].y2[j] = 0;
              }
          } // nLines
         } // frN
}
void cutNofLines(STRUCTWB *lpWorkBuffer, int lNum){
  int  i, j, frN, nLines;
     if(lNum < 0) return;
     frN=(lpWorkBuffer->frNumber);
     for(i=0;i<frN;i++) {
...
int isMainLineOld(STRUCTWB *lpWorkBuffer){
  int  i, j, lm, frN, maxY,tHh;
  int  EndOfLbox, nFr, maxAvLHigh, avLHigh, indMaxAv, indFirst;

     frN=(lpWorkBuffer->frNumber);
     indFirst=-1;
     tHh=max((lpWorkBuffer->  lettWidth)/6,10);

     for(i=0;i<frN;i++) {

         infFrameWork[i].x1=(lpWorkBuffer->infFrame[i]).x1;
         infFrameWork[i].x2=(lpWorkBuffer->infFrame[i]).x2;
         infFrameWork[i].nLines=(lpWorkBuffer->infFrame[i]).nLines;
         for(j=0;j<infFrameWork[i].nLines;j++) {
               lineWork[i].y1[j]=(lpWorkBuffer->line[i]).y1[j];
               lineWork[i].y2[j]=(lpWorkBuffer->line[i]).y2[j];
               }
        }
     maxAvLHigh=0;
     indMaxAv=0;
     ...
     j=0;
     while(!EndOfLbox) {
        if( (nFr=maxYstruct(lpWorkBuffer))>=0 )  {
        ...
        avLHigh=0;
        for(i=0;i<frN;i++)  {
          if( ((lpWorkBuffer->curLine[i]).y2)!=0 ) {
               lm++;
               avLHigh+=(lpWorkBuffer->curLine[i]).y2-(lpWorkBuffer->curLine[i]).y1;
               }
          }
       if(lm>0) {
          avLHigh/=lm;
          }
       if((avLHigh>tHh)&&(indFirst<0)) indFirst=j;
       if(maxAvLHigh<avLHigh) {
          maxAvLHigh=avLHigh;
          indMaxAv=j;
          }
        j++;
     ...
     for(i=0;i<frN;i++) {
         (lpWorkBuffer->infFrame[i]).x1=infFrameWork[i].x1;
         (lpWorkBuffer->infFrame[i]).x2=infFrameWork[i].x2;
         (lpWorkBuffer->infFrame[i]).nLines=infFrameWork[i].nLines;
         for(j=0;j<infFrameWork[i].nLines;j++) {
               (lpWorkBuffer->line[i]).y1[j]=lineWork[i].y1[j];
               (lpWorkBuffer->line[i]).y2[j]=lineWork[i].y2[j];
               }
        }
     indFirst=indMaxAv;
     for(j=0;j<=indFirst;j++) {
        if( (nFr=maxYstruct(lpWorkBuffer))>=0 )  {
                searchBeg(lpWorkBuffer,nFr);
                searchEnd(lpWorkBuffer,nFr);
                }
     ...
     for(i=0;i<frN;i++)  {
        if( ((lpWorkBuffer->curLine[i]).y2)!=0 ) {
           if(maxY>(lpWorkBuffer->curLine[i]).y1) maxY=(lpWorkBuffer->curLine[i]).y1;
           }
        }
     if(maxY==32000) maxY=0;
     (lpWorkBuffer -> trush) = maxY;
    return(maxY);
} /* of isMainLineOld() */

/***************************************************************************
 *  PURPOSE      :search main line                                         *
 ***************************************************************************/
int isMainLine(STRUCTWB * lpWorkBuffer){
  int  i, j, k, h, frN, maxH, nLines;
     frN=(lpWorkBuffer->frNumber);
     for(i=0;i<frN;i++) {
         nLines = (lpWorkBuffer->infFrame[i]).nLines;
         if(nLines > 1) {
            ...
            k    = 0;
            for(j=0;j<nLines;j++) {
              h = (lpWorkBuffer->line[i]).y2[j] - (lpWorkBuffer->line[i]).y1[j];
              if(maxH < h) {maxH = h;k = j;}
              }
            (lpWorkBuffer->line[i]).y1[0] = (lpWorkBuffer->line[i]).y1[k];
            (lpWorkBuffer->line[i]).y2[0] = (lpWorkBuffer->line[i]).y2[k];
     ...
     return( isMainLineOld(lpWorkBuffer));
} /* of isMainLine() */
