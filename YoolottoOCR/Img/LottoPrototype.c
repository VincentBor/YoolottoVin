...
//     	printf(" free zone= %d\n", i);
                free (ticketDataPtr->zone[i].ptrZoneStruct);
            }
...
//    LPTICKETDATA ticketDataPtr = *pticketDataPtr;
    if ( hPtr == NULL){
    	printf("---reallocMemoryBuffer, hptr == NULL !!!!!!");
    	return -1;
    }
//printf("---try to reallocMemoryBuffer, hPtr->size= %ld, size= %u\n", hPtr->size, size);
    if ( hPtr->ptr == NULL || hPtr->size < size) {
//printf("---reallocMemoryBuffer, from  hPtr->size= %ld;  to size= %u\n", hPtr->size, size);
   	     lpPtr = calloc(size, sizeof(BYTE));
  	        if(lpPtr == NULL) {
  	  	       return -1;
  	        }
...
//  printf("file %s length= %d\n",fileName, len);
  ...
  if(ptr == NULL) {
  	sprintf(ticketDataPtr->error,"LoadPrototype (\"%s\") calloc failed: %s\n",fileName, strerror(errno));
  	return -1;
  }
...
workAreaPtr = calloc(sizeWorkAreaAlloc, sizeof(BYTE));
if(workAreaPtr == NULL) {
	sprintf(img->error,"ticketRecognitionInit (\"%s\") calloc failed: %s\n",img->szIMGName, strerror(errno));
	return -1;
}


//*workAreaOutPtr = workAreaPtr; // ??????
...
iDStructPtr = calloc(sizeIDStructAlloc, sizeof(BYTE));
if(iDStructPtr == NULL) {
	sprintf(img->error,"ticketRecognitionInit (\"%s\") calloc failed: %s\n",img->szIMGName, strerror(errno));
	return -1;
}
...
//if(IDVersion>=2)
//  {err = getMemoryHuge(phgIDStruct+1L, sizeIDStructAlloc, 4000L);
//      if(err < 0)
//          {GlobalUnlock(hgWorkArea); GlobalLock(hgIDStruct); return(-101);}
//   hgIDStructF = *(phgIDStruct+1L);
//   lpIDStructF = (BYTE huge *)GlobalLock(hgIDStructF);
//   lpwIDStructF = (WORD huge *)(lpIDStructF + 8L);
//  }
...
//printf("  radonStruct->teta= %d \n", radonStruct->teta);
   	  ...
   	  if(retVal < 0)
   		    {freeh(workAreaOutPtr);free(iDStructPtr);
   		     *iDStructOutPtr = NULL;
    	     return(retVal);
    	    }
}
...
//if(IDVersion>=2)
//  {*(lpwIDStructF + shift + 0L) = 2*(sizeDTetaX + 3);
//   *(lpwIDStructF + shift + 1L) = sizeDTetaX;
//   *(lpwIDStructF + shift + 2L) = (WORD)(dMinTetaX/SCALE);
//   *(lpwIDStructF + shift + 3L) = stepD;

//   for(i=0; i<sizeDTetaX; i++)
//      *(lpwIDStructF+ shift + (DWORD)(4+i)) =
//                *(lpwIDStruct+ shift + (DWORD)(3+sizeDTetaX-i));
//  }
...
//if(IDVersion>=2)
//  {*(lpwIDStructF + shift + 0L) = 2*(sizeDTetaY + 3);
//   *(lpwIDStructF + shift + 1L) = sizeDTetaY;
//   *(lpwIDStructF + shift + 2L) = (WORD)(dMinTetaY/SCALE);
//   *(lpwIDStructF + shift + 3L) = stepD;

//   for(i=0; i<sizeDTetaY; i++)
//      *(lpwIDStructF+ shift + (DWORD)(4+i)) =
//                *(lpwIDStruct+ shift + (DWORD)(3+sizeDTetaY-i));
//  }

shift = shift + (sizeDTetaY + 4);

//printf("  shift= %d, KLEVELS= %d \n",shift,KLEVELS );
...
// free(workAreaPtr);free(iDStructPtr); ???
}
...
void ReleasePrototypes (LPTICKETDATA ticketDataPtr){
    ...
    if(ticketDataPtr->ptrLink)
    	free(ticketDataPtr->ptrLink);
    if(ticketDataPtr->ppIDStruct0)
    	free(ticketDataPtr->ppIDStruct0);
...
//    saveLog("\n--------read OK, size= ", buff);
    ...
    // read OK, check if it is fdf-file
...
//    ReleaseProtChain (ticketDataPtr);    // release previous chain, if any
    ...
    ptrLink = calloc(PROTBUFF, sizeof(BYTE));
    if(ptrLink == NULL) {
//    	printf("calloc failed");
    	sprintf(ticketDataPtr->error,"LoadProtDChain:  calloc failed\n");
    	return -1;
    }
    ticketDataPtr->ptrLink = ptrLink;
    *(int*) ptrLink = 6;  // offset to 1-st prototype
//    *(LPWORD)(ptrLink + 4) = ptrLink;
    ptrBuf = calloc(TMPBUFSIZE, sizeof(BYTE));
...
//printf("ticketDataPtr->numDescr: %d\n", ticketDataPtr->numDescr);
   return i;
}
...
//printf("  sizeTicketStructAlloc= %d \n",sizeTicketStructAlloc);
       ...
       ticketStructPtr = calloc(sizeTicketStructAlloc, sizeof(BYTE));
...
//  printf("  radonStruct->teta= %d \n", radonStruct->teta);
   ...
   if(retVal < 0)
	    {freeh(pWorkAreaPtr);free(ticketStructPtr);
	    *pTicketStruct = NULL;
     	  return(retVal);
      	 }
...
int IdentifyTicketCall (LPRADON radonStruct, HLPBYTE* pWorkAreaPtr, LPTICKETDATA ticketDataPtr, LPBYTE iDStructPtr){
    ...
    ticketDataPtr->ptrOut = calloc(3100 * 2, sizeof(int));
        ...
        {for(j=0; j<wNin; j++) d_PRT[j]=1001;
         ...
         for(i=0; i<wNin; i++){
             ...
             if(((sizeDX0 < 3*sizeDX1)&&(sizeDX1 < 3*sizeDX0)&&
                ...
                (sizeDY0 < 3*sizeDX1)&&(sizeDX1 < 3*sizeDY0)))
                {if(flip_PRT[i] == 0)
                 ...
                 //---------------------------------------------
//                sprintf(buff,"distX= %d, distY= %d, i= %d\n",dist, distY, i);
//                saveLog(" ", buff);
                 //---------------------------------------------
...
//	 sinM = (long)((sin(aPhiM*PI/1800))*SCALE);
	 ...
	 radonStruct.sizeD     = (WORD)((radonStruct.dMax - radonStruct.dMin)/radonStruct.stepDScale + 1L);
	 radonStruct.stepDHalf = ((double)radonStruct.stepD + 1.0)/2;
	 ...
	 recalculateAllocBuffer(&sizeWorkAreaAlloc, 300000);
	 lpWorkArea = calloc(sizeWorkAreaAlloc, sizeof(BYTE));
	 if(lpWorkArea == NULL) {
	       	return -1;
     }
	 {long kk;
